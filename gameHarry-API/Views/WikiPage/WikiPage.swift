...
struct WikiPage<T: Decodable & Identifiable>: View {
    ...
    @ObservedObject var viewModel: WikiViewModel<T>
    ...
    var body: some View {
        ...
        .task {
            await viewModel.loadData(from: endpoint)
        }
    ...
    private var headerView: some View {
        Text(endpoint.path.capitalized).font(.cinzelDecorative(.bold))
            .padding(2)
    ...
    private var itemsGridView: some View {
        ScrollView {
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 20) {
                ForEach(viewModel.filteredItems(searchItem)) { item in
                    returnCard(item)
                }
            }
        }
    ...
    private func returnCard(_ item:T) -> some View {
        if let house = item as? House {
            return AnyView(CardView(cardImage: "globe",
                                    name: house.name,
                                    subname: house.founder))
        } else if let potion = item as? Potion {
            return AnyView(PotionCardView(potion: potion,
                                          potionsFromPotterDB: viewModel.potionsFromPotterDB))
        } else if let spell = item as? Spell {
            return AnyView(CardView(cardImage: "globe",
                                    name: spell.name,
                                    subname: spell.effect ?? "No effect found"))
        } else if let wizard = item as? Wizard {
            return AnyView(CardView(cardImage: "globe",
                                    name: wizard.firstName ?? "None",
                                    subname: wizard.lastName))
        } else {
            return AnyView(CardView(cardImage: "person.fill",
                                    name: "None",
                                    subname: "None"))
        }
    }
